function go_signin() {
    swal({
        ...
        if (isConfirm) {
            setTimeout(function () {
                window.location.href = "form/loginAndRegister.htm";
            }, 300);
        } else {
            window.location.href = "form/loginAndRegister.htm";
        }
...
$("#comment-form .ladda-button").on('click', function (e) {
    ...
    if (form.checkValidity()) {
        ...
        $.post("/bbs/json/TopicRequest/addComment.json", formData, function (result) {
            setTimeout(function () {
                l.stop();
            }, 600);
            if (result) {
...
topic.comment = function () {
    var formData = $('#comment-form').serialize();
    $.post("/bbs/json/TopicRequest/addComment.json", formData, function (response) {
        if (response) {
            if (response.status == 200) {
                window.location.reload();
            } else if (response.status == 401) {
